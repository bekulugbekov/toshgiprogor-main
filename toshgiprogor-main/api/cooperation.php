<?php
// E6: Hamkorlik forma — server-side xavfsizlik (honeypot, rate-limit, validatsiya, 4-til)
// contact.php asosida — cooperation maydonlariga moslashtirilgan.
...
$MESSAGES = [
    'uz' => [
        'success'      => "So'rovingiz yuborildi! Biz 1–2 ish kuni ichida javob beramiz.",
        'errorCompany' => "Kompaniya nomi kamida 2 belgidan iborat bo'lishi kerak.",
        'errorContact' => "Aloqa shaxsi ismini kiriting (kamida 2 belgi).",
        'errorPhone'   => "Telefon raqami noto'g'ri kiritilgan.",
        'errorEmail'   => "Elektron pochta manzili noto'g'ri.",
        ...
        'errorServer'  => "Server xatosi. Iltimos, info@tashgiprogor.uz ga yozing.",
    ],
    'ru' => [
        'success'      => 'Ваш запрос успешно отправлен! Мы ответим в течение 1–2 рабочих дней.',
        'errorCompany' => 'Название компании должно содержать не менее 2 символов.',
        'errorContact' => 'Укажите контактное лицо (не менее 2 символов).',
        ...
        'errorServer'  => 'Ошибка сервера. Напишите нам на info@tashgiprogor.uz.',
    ],
    'en' => [
        'success'      => 'Your enquiry has been submitted! We will reply within 1–2 business days.',
        'errorCompany' => 'Company name must be at least 2 characters.',
        'errorContact' => 'Please provide a contact person (at least 2 characters).',
        ...
        'errorServer'  => 'Server error. Please email info@tashgiprogor.uz.',
    ],
    'zh' => [
        'success'      => '您的咨询已成功提交！我们将在1–2个工作日内回复。',
        'errorCompany' => '公司名称至少需要2个字符。',
        'errorContact' => '请提供联系人姓名（至少2个字符）。',
...
function checkRateLimit($ip, $logsDir) {
    ...
    $file   = $logsDir . '/rate_limit_coop.json';
    $now    = time();
    $window = 3600;
    $max    = 5;
...
$projectType = trim(strip_tags($data['projectType'] ?? ''));
$phone       = trim(strip_tags($data['phone']       ?? ''));
$email       = trim(strip_tags($data['email']       ?? ''));
$message     = trim(strip_tags($data['message']     ?? ''));
$privacy     = ($data['privacy'] ?? '0') === '1' || $data['privacy'] === true;

if (mb_strlen($company) < 2)                          sendJson(false, $msgs['errorCompany']);
if (mb_strlen($contact) < 2)                          sendJson(false, $msgs['errorContact']);
if (mb_strlen(preg_replace('/\D/', '', $phone)) < 7)  sendJson(false, $msgs['errorPhone']);
if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) sendJson(false, $msgs['errorEmail']);
if (!$privacy)                                         sendJson(false, $msgs['errorConsent']);

// Uzunlik cheklovi
$message     = mb_substr($message, 0, 2000);
$company     = mb_substr($company, 0, 200);
$contact     = mb_substr($contact, 0, 200);
$projectType = mb_substr($projectType, 0, 200);

// Log yozish (zaxira)
function writeLog($entry, $logsDir) {
    if (!is_dir($logsDir)) @mkdir($logsDir, 0750, true);
    $file = $logsDir . '/cooperation.log';
...
$text  = "🤝 <b>Hamkorlik so'rovi / Запрос на сотрудничество</b>\n\n";
$text .= "🏢 <b>Kompaniya / Компания:</b> " . htmlspecialchars($company, ENT_QUOTES) . "\n";
$text .= "👤 <b>Aloqa shaxsi / Контакт:</b> " . htmlspecialchars($contact, ENT_QUOTES) . "\n";
if ($projectType !== '') {
    $text .= "🏗 <b>Loyiha turi / Тип проекта:</b> " . htmlspecialchars($projectType, ENT_QUOTES) . "\n";
}
...
// Log saqlandi — Telegram ishlamasa ham foydalanuvchiga muvaffaqiyat qaytariladi
sendJson(true, $msgs['success']);
